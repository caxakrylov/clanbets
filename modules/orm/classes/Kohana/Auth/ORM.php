...
class Kohana_Auth_ORM extends Auth {
	 ...
	 * @return  boolean
	 */
	protected function _login_email($email, $password, $remember)
	{
		if ( ! is_object($email))
		{
			...
			$email->where($email->unique_key($username_email), '=', $username_email)->find();
		}
		...
		// If the passwords match, perform a login
		if ($email->has('roles', ORM::factory('Role', array('name' => 'login'))) AND $email->password === $password)
		{
			if ($remember === TRUE)
			{
				// Token data
